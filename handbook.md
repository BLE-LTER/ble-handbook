...
Run `bleutils::init_script()` in any R console and specify that `type = "update"`, e.g., for dataset 15 with data updates from 2022, `bleutils::init_script(15, file.path(getwd(), as.integer(2022)), type = "update")`. Note that for `init_datapkg()` also calls `init_script()` but with `type = "init"` instead. So, you can also use that option outside of initiating a whole new package.
